...
def do_first(start_flag):
    ...
    for i in range(3):
        ...
        if position == 0:
            ...
            pyautogui.click(1473,1144)    pyautogui.scroll(-1)
...
while True:
   ...
   le_module_image_np, cv2.COLOR_BGR2RGB)
        single_module_image_with_pixels[pixel_y, pixel_x] = (0, 0, 255)
        single_module_image_with_pixels[pixel_y2, pixel_x2] = (255, 0, 0)
...
def reaction_test():
    ...
    while move < 5:
        # time.sleep(0.1)
        print(pyautogui.pixel(200, 200))
        # blue (43, 135, 209)
        # red (206, 38, 54)
        # green (75, 219, 106)
        if pyautogui.pixel(200, 200) == (75, 219, 106):
            pyautogui.click()
            time.sleep(1)
            pyautogui.click()
            move += 1
...
def press_array(pos_array):
    ...
    for pos in pos_array:
        if pos in positions:
            pyautogui.click(*positions[pos])


def sequence():
    level1 = [["" for _ in range(3)] for _ in range(3)]
    positions = {
        '1': (1131, 295),
        '2': (1252, 295),
        '3': (1386, 295),
    ...
    # positons:
    # 1 - (1131 295) 2 - (1252 295) 3 - (1386 295)
    # 4 - (1131 432) 5 - (1252 432) 6 - (1386 432)
    # 7 - (1131 559) 8 - (1252 559) 9 - (1386 559)

    # movement
    pyautogui.click(1254, 535)  # press on start button
    time.sleep(1)
    level1_press = []  # what to press in which order, plan is to have numbers from 1 to 9 here

    for i in range(1, 99):  # i think level 1 lenghts is 10

        for j in range(i):

            print("starting time sleep")
            a = 0
            if i > 30:
                a = 0.1
                if i > 45:
                    ...
                    if i > 70:
                        a = 0.7
            time.sleep(0.5 * i + a)
            print("ending time sleep")

            if pyautogui.pixel(1131, 295) != (37, 115, 193):
                level1_press.append("1")
                print("number 1 is white")
                break
            elif pyautogui.pixel(1252, 295) != (37, 115, 193):
                level1_press.append("2")
                print("number 2 is white")
                break
            elif pyautogui.pixel(1386, 295) != (37, 115, 193):
                level1_press.append("3")
                print("number 3 is white")
                break
            elif pyautogui.pixel(1131, 432) != (37, 115, 193):
                level1_press.append("4")
                print("number 4 is white")
                break
            elif pyautogui.pixel(1252, 432) != (37, 115, 193):
                level1_press.append("5")
                print("number 5 is white")
                break
            elif pyautogui.pixel(1386, 432) != (37, 115, 193):
                level1_press.append("6")
                print("number 6 is white")
                break
            elif pyautogui.pixel(1131, 559) != (37, 115, 193):
                level1_press.append("7")
                print("number 7 is white")
                break
            elif pyautogui.pixel(1252, 559) != (37, 115, 193):
                level1_press.append("8")
                print("number 8 is white")
                break
            elif pyautogui.pixel(1386, 559) != (37, 115, 193):
                level1_press.append("9")
                print("number 9 is white")
                break

        print(level1_press)

        press_array(level1_press)
        time.sleep(1)
...
def show_pos():
    while True:
        ...
        print(f"position is : x={x}, y={y} {pyautogui.pixel(x, y)}")
        time.sleep(0.5)
...
def chimp():
    ...
    for i in range(4, 41):
        board = []
        pyautogui.click(1250, 540)
        for j in range(i):
            '''
            position is : x=890, y=170 (43, 135, 209) top left
            position is : x=1618, y=170 (43, 135, 209) top right
            position is : x=890, y=635 (43, 135, 209) bottom left
            position is : x=1618, y=635 (43, 135, 209) bottom right

            '''

            button_pos = pyautogui.locateOnScreen(f'C:\\Users\\alexx\\Desktop\\chimp\\{j + 1}.png', region=region)
            print(f"appending {j + 1}.png j:{j}")
            board.append(button_pos)
        print(i, j, board)
        for pos in board:
            print(pos[0] + int(pos[2] / 2), pos[1] + int(pos[3] / 2), 'this pos we wanna press now ')
            try:
                pyautogui.click(pos[0] + int(pos[2] / 2), pos[1] + int(pos[3] / 2))
            except ImageNotFoundException:
                pass


def aim_loop():
    # needs to be played at 25% site scale and with f12 on because thats how i coded it xd
    bg_color = (209, 135, 43)
    import numpy as np
    import mss
    pyautogui.PAUSE = 0.0
    ...
    with mss.mss() as sct:
        ...
        # Przechwyć ekran w określonym regionie
        screenshot = sct.grab((866, 110, 1085, 225))
        ...
        # Rysujemy czerwone kropki na wykrytych pikselach
        for y in range(0, img_array.shape[0], 10):
            for x in range(0, img_array.shape[1], 10):
                pixel = tuple(img_array[y, x, :3])
                if pixel != bg_color:
                    # draw.ellipse((x - 1, y - 1, x , y ), fill="red")
                    # print(x,y)
                    pyautogui.click(866 + x, 110 + y)
                    time.sleep(0.01)
                    return


def aim():
    pyautogui.click(976, 158)

    # 866 100 top left
    # 1085 210 bottom right
...
def typing():
    ...
    bbox = (775, 373, 1728, 530)
    screenshot = ImageGrab.grab(bbox)

    tekst = pytesseract.image_to_string(screenshot, lang='eng')
...
def number():
    # works at 200% of page
    import random
    pyautogui.click(1255, 1020)
    time.sleep(0.4)
    pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files\Tesseract-OCR\tesseract.exe'
    bbox = (284, 373, 2222, 1020)
...
def words():
    pyautogui.click(1250, 1050)  # start
    bbox = (860, 636, 1636, 780)
    # also works at 200% of page
    ar = []
    pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files\Tesseract-OCR\tesseract.exe'
    ...
    for i in range(1001):
        ...
        if tekst not in ar:
            ar.append(tekst)
            pyautogui.click(1400, 880)
        else:
            pyautogui.click(1100, 880)
        # time.sleep(0.1)
    # 1950 800
    # 750 600
    # 1100 880 seen
...
# Ścieżka do folderu "obr" na pulpicie
desktop_path = os.path.join(os.path.expanduser("~"), "Desktop")
obr_folder = os.path.join(desktop_path, "obr")

# Utwórz folder "obr", jeśli nie istnieje
if not os.path.exists(obr_folder):
    os.makedirs(obr_folder)

def get_next_filename(folder):
    """Funkcja zwracająca kolejną dostępną nazwę pliku w folderze."""
    existing_files = os.listdir(folder)
    existing_numbers = [int(f.split(".")[0]) for f in existing_files if f.endswith(".png") and f.split(".")[0].isdigit()]
    next_number = max(existing_numbers) + 1 if existing_numbers else 1
    return os.path.join(folder, f"{next_number}.png")
def visual():
    # works on other monitor
    pyautogui.click(1250, 1150)
    time.sleep(0.7)
    bbox = (700, 412, 1756, 1380)
    # screenshot = ImageGrab.grab(bbox)
    ...
    for i in range(101):
        ...
        if i < 2:
            ...
            filename = get_next_filename(obr_folder)
            ...
            for x in range(3):
                for y in range(3):
                    print(x * 300 + 860, y * 300 + 520, pyautogui.pixel(x * 300 + 860, y * 300 + 520))
                    cv2.circle(screenshot, (x * 300 + 860 - bbox[0], y * 300 + 520 - bbox[1]), 5, (0, 0, 255), -1)

                    if pyautogui.pixel(x * 300 + 860, y * 300 + 520) == (255, 255, 255):
                        press.append((x * 300 + 860, y * 300 + 520))
            print("\n")
            time.sleep(1.2)
            for pos in press:
        ...
        if i > 1 and i < 5:
            ...
            # Definiowanie bounding box
            bbox = (700, 412, 1756, 1380)

            # Przechwyć screenshot
            screenshot = pyautogui.screenshot(region=bbox)
            screenshot = np.array(screenshot)
            screenshot = cv2.cvtColor(screenshot, cv2.COLOR_BGR2RGB)

            # Zapisz screenshot do folderu "obr"
            filename = get_next_filename(obr_folder)
            cv2.imwrite(filename, cv2.cvtColor(screenshot, cv2.COLOR_RGB2BGR))

            # Wczytaj screenshot z pliku
            loaded_screenshot = cv2.imread(filename)
            loaded_screenshot = cv2.cvtColor(loaded_screenshot, cv2.COLOR_BGR2RGB)  # Konwersja do RGB

            # Przetwarzanie wczytanego screenshotu
            press = []
            xpos = 840
            ypos = 510
            multi = 240  # px how much squares are in lenght
            size = 4

            for x in range(size):
                for y in range(size):
                    # Oblicz współrzędne piksela względem bounding box
                    pixel_x = x * multi + xpos - bbox[0]
                    pixel_y = y * multi + ypos - bbox[1]

                    # Pobierz kolor piksela z wczytanego screenshotu
                    pixel_color = loaded_screenshot[pixel_y, pixel_x]  # Uwaga: numpy używa indeksowania (y, x)

                    # Narysuj czerwoną kropkę na obrazku
                    cv2.circle(loaded_screenshot, (pixel_x, pixel_y), 5, (0, 0, 255), -1)

                    # Sprawdź, czy kolor piksela to (255, 255, 255) (biały)
                    if np.array_equal(pixel_color, [255, 255, 255]):
                        press.append((x * multi + xpos, y * multi + ypos))

            # Zapisz przetworzony screenshot z czerwonymi kropkami do folderu "obr"
            processed_filename = get_next_filename(obr_folder)
            cv2.imwrite(processed_filename, cv2.cvtColor(loaded_screenshot, cv2.COLOR_RGB2BGR))

            # Wyświetl przetworzony screenshot


            # Kliknięcie w zaznaczone miejsca
            print("\n")
            time.sleep(1.2)
            for pos in press:
                pyautogui.click(pos[0], pos[1])
        ...
        if 5 < i < 9:
            ...
            xpos = 830
            ypos = 500
            multi = 180  # px how much squares are in lenght
            size = 5
            print(size)
            for x in range(size):
                for y in range(size):
                    ...
                    # Pobierz kolor piksela z screenshot
                    pixel_color = screenshot[pixel_y, pixel_x]  # Uwaga: numpy używa indeksowania (y, x)

                    # Narysuj czerwoną kropkę na obrazku
                    cv2.circle(screenshot, (pixel_x, pixel_y), 5, (0, 0, 255), -1)

                    # Sprawdź, czy kolor piksela to (255, 255, 255) (biały)
                    if np.array_equal(pixel_color, [255, 255, 255]):
                        press.append((x * multi + xpos, y * multi + ypos))

            # Zapisz screenshot z czerwonymi kropkami do folderu "obr"
            filename = get_next_filename(obr_folder)
            cv2.imwrite(filename, cv2.cvtColor(screenshot, cv2.COLOR_RGB2BGR))
        ...
        if 8 < i < 14:
            ...
            xpos = 830
            ypos = 500
            multi = 150  # px how much squares are in lenght
            size = 6
            print(size)
            for x in range(size):
                for y in range(size):
        ...
        if 14 < i < 20:
            ...
            xpos = 830
            ypos = 500
            multi = 130  # px how much squares are in lenght
            size = 7
            print(size)
            for x in range(size):
                for y in range(size):
        ...
        if 19 < i < 25:
            ...
            xpos = 830
            ypos = 500
            multi = 110  # px how much squares are in lenght
            size = 8
            print(size)
        ...
        if 24 < i < 32:
            xpos = 830
            ypos = 500
            multi = 100  # px how much squares are in lenght
            size = 9
            print(size)
            ...
            # Sprawdzaj piksele na screenshot, a nie na aktualnym ekranie
            for x in range(size):
                for y in range(size):
                    # Oblicz współrzędne piksela względem bounding box
                    pixel_x = x * multi + xpos - bbox[0]
        ...
        if 31 < i < 39:
            xpos = 830
            ypos = 500
            multi = 90  # px how much squares are in lenght
            size = 10
            print(size)
            for x in range(size):
                for y in range(size):
...
def detect_white_tiles(stage,size, xpos, ypos, tiles, delay):
    """
    Wykrywa i klika w białe kafelki na planszy gry.
    :param size: Rozmiar pojedynczego kafelka (px)
    :param xpos: Pozycja X lewego górnego rogu planszy
    :param ypos: Pozycja Y lewego górnego rogu planszy
    :param tiles: Liczba kafelków w wierszu/kolumnie
    :param delay: Opóźnienie po kliknięciu (sekundy)
    """
    region = (137,446,1060-137,1365-446)
    ...
    for xblock in range(tiles):
        for yblock in range(tiles):
            x = xpos-137+xblock*size
            y = ypos-446+yblock*size
            r, g, b = image[y, x]  # Pobranie koloru piksela
            if r > 240 and g > 240 and b > 240:  # Sprawdzenie, czy to biały kolor
                press.append(( x, y))
    ...
    time.sleep(1.2)
    for pos in press:
        pyautogui.click(pos[0]+137, pos[1]+446)
        #ime.sleep(delay)  # Opóźnienie między kliknięciami

def visual2():
    #works on second monitor with win+left so window is hald the size
    #137 446
    #1060 1365
    # i give up on first one xd
    bbox = (137,446,1060-137,1365-446)
    ...
    for i in range(1,101):
        ...
        cv2.waitKey(0) # dziala dla stage1'''
        print("ASDFGHJ",i)
        if i < 3:
            tiles = 3 # 3x3 board for stage lower than 3
            xpos,ypos = 305,600 # pos of upper left tile
            size = 260 # size of tile
            '''for xblock in range(tiles):
                for yblock in range(tiles):
                    if pyautogui.pixel(xpos+xblock*size,ypos+yblock*size) == (255,255,255):
                        press.append((xpos + xblock * size, ypos + yblock * size))
                        print("X", end='')
                    else: print("O", end='')
                print()
            time.sleep(1) # wait for squares to dissapear, so you can input button presses
            for pos in press:
                pyautogui.click(pos[0],pos[1])'''
            detect_white_tiles(i,size, xpos, ypos, tiles, 2.3)
            time.sleep(2.3) # break between stages

        if 2 < i < 6:
            tiles = 4 # 3x3 board for stage lower than 3
            xpos,ypos = 260,570 # pos of upper left tile
            size = 200 # size of tile
            '''for xblock in range(tiles):
                for yblock in range(tiles):
                    if pyautogui.pixel(xpos+xblock*size,ypos+yblock*size) == (255,255,255):
                        press.append((xpos + xblock * size, ypos + yblock * size))
            ...
            for pos in press:
                pyautogui.click(pos[0],pos[1])'''
            detect_white_tiles(i,size, xpos, ypos, tiles, 2.3)
            time.sleep(2.2) # break between stages
        if 5 < i < 9:
            tiles = 5 # 3x3 board for stage lower than 3
            xpos,ypos = 240,550 # pos of upper left tile
            size = 175 # size of tile
            '''for xblock in range(tiles):
                for yblock in range(tiles):
                    if pyautogui.pixel(xpos+xblock*size,ypos+yblock*size) == (255,255,255):
                        press.append((xpos + xblock * size, ypos + yblock * size))
                ...
                print()
            print("break to make squares disapear")
            time.sleep(1)  # wait for squares to dissapear, so you can input button presses
            print("doing moves")
            for pos in press:
                pyautogui.click(pos[0], pos[1])
            print("break between stages")'''
            detect_white_tiles(i,size, xpos, ypos, tiles, 2.3)
            time.sleep(2.2) # break between stages
        if 8 < i < 14:
            tiles = 6 # 3x3 board for stage lower than 3
            xpos,ypos = 230,540 # pos of upper left tile
            size = 150 # size of tile
            '''for xblock in range(tiles):
                for yblock in range(tiles):
                    if pyautogui.pixel(xpos+xblock*size,ypos+yblock*size) == (255,255,255):
                        press.append((xpos + xblock * size, ypos + yblock * size))
            ...
            time.sleep(2.2) # break between stages
        if 13 < i < 19:
            tiles = 7 # 3x3 board for stage lower than 3
            xpos,ypos = 220,530 # pos of upper left tile
            size = 125 # size of tile
            '''for xblock in range(tiles):
                for yblock in range(tiles):
                    if pyautogui.pixel(xpos+xblock*size,ypos+yblock*size) == (255,255,255):
                        press.append((xpos + xblock * size, ypos + yblock * size))
            ...
            print("break between stages")'''
            detect_white_tiles(i,size, xpos, ypos, tiles, 2.3)
            time.sleep(2.0) # break between stages
        if 18 < i < 24:
            tiles = 8 # 3x3 board for stage lower than 3
            xpos,ypos = 210,520 # pos of upper left tile
            size = 110 # size of tile
            '''print("entering loop")
            for xblock in range(tiles):
                for yblock in range(tiles):
                    if pyautogui.pixel(xpos+xblock*size,ypos+yblock*size) == (255,255,255):
            ...
            print("break between stages")'''
            detect_white_tiles(i,size, xpos, ypos, tiles, 2.3)
            time.sleep(1.7) # break between stages
        if 23 < i < 31:
            tiles = 9 # 3x3 board for stage lower than 3
            xpos,ypos = 210,520 # pos of upper left tile
            size = 97 # size of tile # slightly off but worked so far 100%
            ''' for xblock in range(tiles):
                for yblock in range(tiles):
                    if pyautogui.pixel(xpos+xblock*size,ypos+yblock*size) == (255,255,255):
                        press.append((xpos + xblock * size, ypos + yblock * size))
            ...
            print("break between stages")'''
            detect_white_tiles(i,size, xpos, ypos, tiles, 2.3)
            time.sleep(1.80) # break between stages
        if 30 < i < 38:
            tiles = 10 # 3x3 board for stage lower than 3
            xpos,ypos = 200,510 # pos of upper left tile
            size = 88 # size of tile # slightly off but worked so far 100%
            '''for xblock in range(tiles):
                for yblock in range(tiles):
                    if pyautogui.pixel(xpos+xblock*size,ypos+yblock*size) == (255,255,255):
                        press.append((xpos + xblock * size, ypos + yblock * size))
        ...
        if 37 < i < 45:
            tiles = 11 # 3x3 board for stage lower than 3
            xpos,ypos = 205,515 # pos of upper left tile
            size = 79 # size of tile # slightly off but worked so far 100%
            '''for xblock in range(tiles):
                for yblock in range(tiles):
                    if pyautogui.pixel(xpos+xblock*size,ypos+yblock*size) == (255,255,255):
                        press.append((xpos + xblock * size, ypos + yblock * size))
            ...
            for pos in press:
                pyautogui.click(pos[0],pos[1])
            time.sleep(1.55) # break between stages
        if 44 < i < 59:
            tiles = 12 # 3x3 board for stage lower than 3
            xpos,ypos = 205,515 # pos of upper left tile
            size = 73 # size of tile # slightly off but worked so far 100%
            for xblock in range(tiles):
                for yblock in range(tiles):
                    if pyautogui.pixel(xpos+xblock*size,ypos+yblock*size) == (255,255,255):
                        press.append((xpos + xblock * size, ypos + yblock * size))
        ...
        if 44 < i < 54:
            ...
            time.sleep(1.7)  # break between stages
        if 53 < i < 63:
            tiles = 13  # 3x3 board for stage lower than 3
            xpos, ypos = 205, 515  # pos of upper left tile
            size = 65  # s
            detect_white_tiles(i, size, xpos, ypos, tiles, 2.3)
            time.sleep(1.7)  # break between stages
        if 62 < i < 72:
            tiles = 14  # 3x3 board for stage lower than 3
            xpos, ypos = 202, 512  # pos of upper left tile
            size = 61 # s
            detect_white_tiles(i, size, xpos, ypos, tiles, 2.3)
            time.sleep(1.7)
        if 71 < i < 83:
            tiles = 15 # 3x3 board for stage lower than 3
            xpos, ypos = 202, 512  # pos of upper left tile
            size = 57 # s
            detect_white_tiles(i, size, xpos, ypos, tiles, 2.3)
            time.sleep(1.7)
        if 82 < i < 94:
            tiles = 16 # 3x3 board for stage lower than 3
            xpos, ypos = 195, 505  # pos of upper left tile
            size = 54 # s
            detect_white_tiles(i, size, xpos, ypos, tiles, 2.3)
            time.sleep(1.7)
        if 93 < i < 999:
            tiles = 17 # 3x3 board for stage lower than 3
            xpos, ypos = 185, 500  # pos of upper left tile
            size = 51 # s
            detect_white_tiles(i, size, xpos, ypos, tiles, 2.3)
            time.sleep(1.7)
def test():
    ...
    xpos, ypos = 185, 500  # pos of upper left tile
    size = 51  # s
    detect_white_tiles(90, size, xpos, ypos, tiles, 2.3)
    time.sleep(1.7)
